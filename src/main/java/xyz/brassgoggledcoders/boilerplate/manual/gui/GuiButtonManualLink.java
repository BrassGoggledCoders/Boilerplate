...
public class GuiButtonManualLink extends GuiButton {
	...
	public void drawButton(Minecraft mc, int mx, int my) {
		if(!gui.manual.showEntryInList(gui.manual.getEntry(key)))
			return;
		this.hovered = mx >= this.xPosition && my >= this.yPosition && mx < this.xPosition + this.width
		...
		if(hovered) {
			...
			gui.drawHoveringText(Arrays.asList(gui.manual.formatEntryName(key) + ", " + (pageLinked + 1)), mx + 8,
					my + 4, font);
