...
import xyz.brassgoggledcoders.boilerplate.manual.gui.GuiButtonManualLink;
import xyz.brassgoggledcoders.boilerplate.manual.gui.GuiButtonManualNavigation;
...
public abstract class ManualPages implements IManualPage {
		...
		public void renderPage(GuiManual gui, int x, int y, int mx, int my) {
			...
			for(int i = 0; i < resources.length; i++)
				if(resources[i] != null && !resources[i].isEmpty()) {
					...
					gui.drawGradientRect(x + xOff - 2, y + yOff - 2, x + xOff + sizing[i][2] + 2,
							y + yOff + sizing[i][3] + 2, 0xffeaa74c, 0xfff6b059);
					gui.drawGradientRect(x + xOff - 1, y + yOff - 1, x + xOff + sizing[i][2] + 1,
							y + yOff + sizing[i][3] + 1, 0xffc68e46, 0xffbe8844);
					yOff += sizing[i][3] + 5;
				}
			String lastResource = "";
			yOff = 0;
			for(int i = 0; i < resources.length; i++)
				if(resources[i] != null && !resources[i].isEmpty()) {
					...
					int xOff = 60 - sizing[i][2] / 2;
					ManualUtils.drawTexturedRect(x + xOff, y + yOff, sizing[i][2], sizing[i][3], (sizing[i][0]) / 256f,
							(sizing[i][0] + sizing[i][2]) / 256f, (sizing[i][1]) / 256f,
			...
			if(localizedText != null && !localizedText.isEmpty())
				ManualUtils.drawSplitString(manual.fontRenderer, localizedText, x, y + yOff, 120,
	...
	public static class Table extends ManualPages {
		...
		public void initPage(GuiManual gui, int x, int y, List<GuiButton> pageButtons) {
			...
			manual.fontRenderer.setUnicodeFlag(true);
			int l = localizedText != null ? manual.fontRenderer.listFormattedStringToWidth(localizedText, 120).size()
					: 0;
		...
		public void renderPage(GuiManual gui, int x, int y, int mx, int my) {
			...
			if(localizedTable != null) {
					...
					if(localizedTable[i] != null)
						for(int j = 0; j < localizedTable[i].length; j++)
							if(localizedTable[i][j] != null) {
								...
								ManualUtils.drawSplitString(manual.fontRenderer, localizedTable[i][j], xx,
										y + textHeight + yOff, w, manual.getTextColour());
				...
				if(bars != null)
					for(int i = 0; i < bars.length; i++)
						gui.drawGradientRect(textOff[i] - 4, y + textHeight - 4, textOff[i] - 3, y + textHeight + yOff,
								col, col);
			}
		}
	...
	public static class ItemDisplay extends ManualPages {
		...
		public void renderPage(GuiManual gui, int x, int y, int mx, int my) {
			...
			if(length > 0) {
				...
				for(int line = 0; line < lines; line++) {
					...
					for(int i = 0; i < perLine; i++) {
						...
						ManualUtils.renderItem().renderItemAndEffectIntoGUI(stacks[item], (int) (xx / scale),
								(int) (yy / scale));
			...
			GlStateManager.enableBlend();
			if(localizedText != null && !localizedText.isEmpty())
				ManualUtils.drawSplitString(manual.fontRenderer, localizedText, x, y + yOffset, 120,
						manual.getTextColour());
	...
	public static class Crafting extends ManualPages {
		...
		void checkRecipe(IRecipe rec, Object key, Object stack, int iStack) {
			if(rec.getRecipeOutput() != null && ManualUtils.stackMatchesObject(rec.getRecipeOutput(), stack)) {
				...
				if(ingredients != null) {
					...
					pIngredients[pIngredients.length - 1] = new PositionedItemStack(rec.getRecipeOutput(),
							xBase + w * 18 + 18, (int) (h / 2f * 18) - 8);
		...
		public void initPage(GuiManual gui, int x, int y, List<GuiButton> pageButtons) {
			...
			for(Object stack : this.stacks) {
				if(this.recipes.get(stack).size() > 1) {
					pageButtons.add(new GuiButtonManualNavigation(gui, 100 * i + 0, x - 2,
							y + yyOff + yOff[i - 1] / 2 - 3, 8, 10, 0));
					pageButtons.add(new GuiButtonManualNavigation(gui, 100 * i + 1, x + 122 - 16,
							y + yyOff + yOff[i - 1] / 2 - 3, 8, 10, 1));
				}
		...
		public void renderPage(GuiManual gui, int x, int y, int mx, int my) {
				...
				if(!rList.isEmpty() && recipePage[i] >= 0 && recipePage[i] < this.recipes.size()) {
					...
					for(PositionedItemStack pstack : rList.get(recipePage[i]))
						if(pstack != null) {
							if(pstack.x > maxX)
								maxX = pstack.x;
							gui.drawGradientRect(x + pstack.x, y + totalYOff + pstack.y, x + pstack.x + 16,
									y + totalYOff + pstack.y + 16, 0x33666666, 0x33666666);
						}
					ManualUtils.bindTexture(manual.texture);
					ManualUtils.drawTexturedRect(x + maxX - 17, y + totalYOff + yOff[i] / 2 - 5, 16, 10, 0 / 256f,
					...
					for(PositionedItemStack pstack : rList.get(recipePage[i]))
						if(pstack != null)
							if(pstack.getStack() != null) {
								ManualUtils.renderItem().renderItemAndEffectIntoGUI(pstack.getStack(), x + pstack.x,
										y + totalYOff + pstack.y);
								ManualUtils.renderItem().renderItemOverlayIntoGUI(manual.fontRenderer,
										pstack.getStack(), x + pstack.x, y + totalYOff + pstack.y, null);
								if(mx >= x + pstack.x && mx < x + pstack.x + 16 && my >= y + totalYOff + pstack.y
										&& my < y + totalYOff + pstack.y + 16)
			...
			manual.fontRenderer.setUnicodeFlag(uni);
			if(localizedText != null && !localizedText.isEmpty())
				ManualUtils.drawSplitString(manual.fontRenderer, localizedText, x, y + totalYOff - 2, 120,
						manual.getTextColour());
	...
	public static class CraftingMulti extends ManualPages {
						...
						for(int iStack = 0; iStack < stacks.length; iStack++) {
							...
							if(((IRecipe) o).getRecipeOutput() != null
								...
								if(r instanceof ShapelessRecipes) {
									...
									w = ReflectionHelper.getPrivateValue(ShapedOreRecipe.class, (ShapedOreRecipe) r,
											"width");
									h = ReflectionHelper.getPrivateValue(ShapedOreRecipe.class, (ShapedOreRecipe) r,
											"height");
								}
								...
								for(int iO = 0; iO < ingredientsPre.length; iO++) {
									if(ingredientsPre[iO] instanceof List) {
										ingredients[iO] = new ArrayList((List) ingredientsPre[iO]);
										Iterator<ItemStack> itValidate =
												((ArrayList<ItemStack>) ingredients[iO]).iterator();
										while(itValidate.hasNext()) {
											ItemStack stVal = itValidate.next();
											if(stVal == null || stVal.getItem() == null
													|| stVal.getDisplayName() == null)
												itValidate.remove();
										}
									}
									else
								...
								if(ingredients != null) {
									PositionedItemStack[] pIngredients =
											new PositionedItemStack[ingredients.length + 1];
									int xBase = (120 - (w + 2) * 18) / 2;
									for(int hh = 0; hh < h; hh++)
										for(int ww = 0; ww < w; ww++)
											if(hh * w + ww < ingredients.length)
												pIngredients[hh * w + ww] = new PositionedItemStack(
														ingredients[hh * w + ww], xBase + ww * 18, hh * 18);
									pIngredients[pIngredients.length - 1] =
											new PositionedItemStack(((IRecipe) o).getRecipeOutput(),
													xBase + w * 18 + 18, (int) (h / 2f * 18) - 8);
		...
		public void renderPage(GuiManual gui, int x, int y, int mx, int my) {
			...
			if(!recipes.isEmpty() && recipePage >= 0 && recipePage < this.recipes.size()) {
				...
				for(PositionedItemStack pstack : recipes.get(recipePage))
					if(pstack != null) {
						if(pstack.x > maxX)
							maxX = pstack.x;
						gui.drawGradientRect(x + pstack.x, y + pstack.y, x + pstack.x + 16, y + pstack.y + 16,
								0x33666666, 0x33666666);
					}
				ManualUtils.bindTexture(manual.texture);
				ManualUtils.drawTexturedRect(x + maxX - 17, y + yOff / 2 - 5, 16, 10, 0 / 256f, 16 / 256f, 226 / 256f,
						236 / 256f);
				...
				for(PositionedItemStack pstack : recipes.get(recipePage))
					if(pstack != null)
						if(pstack.getStack() != null) {
							ManualUtils.renderItem().renderItemAndEffectIntoGUI(pstack.getStack(), x + pstack.x,
									y + pstack.y);
							ManualUtils.renderItem().renderItemOverlayIntoGUI(manual.fontRenderer, pstack.getStack(),
									x + pstack.x, y + pstack.y, null);

							if(mx >= x + pstack.x && mx < x + pstack.x + 16 && my >= y + pstack.y
									&& my < y + pstack.y + 16)
								highlighted = pstack.getStack();
						}
			}
			...
			manual.fontRenderer.setUnicodeFlag(uni);
			if(localizedText != null && !localizedText.isEmpty())
				ManualUtils.drawSplitString(manual.fontRenderer, localizedText, x, y + yOff + 2, 120,
						manual.getTextColour());
		...
		while(itRep.hasNext()) {
			...
			for(int yOff = 0; yOff < list.size(); yOff++) {
				...
				if((start = s.indexOf(rep[0])) >= 0) {
					...
					pageButtons.add(new GuiButtonManualLink(gui, 900 + overflow, x + bx, y + by, bw,
							(int) (helper.fontRenderer.FONT_HEIGHT * 1.5), bkey, rep[0], bpage));
					break;
				}
			}
		}
			...
			if(displayList == null) {
				...
				if(stack instanceof ItemStack) {
					if(((ItemStack) stack).getItemDamage() == OreDictionary.WILDCARD_VALUE) {
						...
						((ItemStack) stack).getItem().getSubItems(((ItemStack) stack).getItem(),
								((ItemStack) stack).getItem().getCreativeTab(), list);
						if(list.size() > 0)
					...
					for(ItemStack subStack : (List<ItemStack>) this.stack) {
						if(subStack.getItemDamage() == OreDictionary.WILDCARD_VALUE) {
							ArrayList<ItemStack> list = new ArrayList<ItemStack>();
							subStack.getItem().getSubItems(subStack.getItem(), subStack.getItem().getCreativeTab(),
									list);
							if(list.size() > 0)
								displayList.addAll(list);
						}
